...
export class ListarProgramacionService {
  async listarProgramaciones() {
    try {
      ...
      // Verificar que los datos de 'clientes' no sean nulos antes de acceder a ellos
      return programacion.map((programaciones) => ({
        ...
        razon_social_cliente: programaciones.clientes
          ? programaciones.clientes.razon_social_cliente
          : "No Cliente", // Si no existe cliente, usar un valor por defecto
        ...
        cantidad_bultos_programacion:
          programaciones.cantidad_bultos_programacion,
...
export class ListarProgramacionService {
  async listarProgramaciones() {
    ...
    // Devolver los datos tal como están, sin usar DTO
    return programacion.map((programaciones) => ({
      id: programaciones.id,
      id_orden_servicio: programaciones.id_orden_servicio,
      id_cliente_programacion: programaciones.id_cliente_programacion,

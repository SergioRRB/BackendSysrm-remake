...
export class ImportarClienteCourrierService {
  async insertClientesCourriers(
    data_ClientesCourriers: ClienteCourrierDto[], idCliente: number, idArea: number, idUser: number) {
    ...
    const cliente = await prisma.clientes.findUnique({where: { id: idCliente } });
    const area = await prisma.areas.findUnique({where: { id: idArea } });
